...
public class MutationPrevalenceDef {
	...
	public static List<Map<String, Object>>
			getBoundMutationPrevalenceList(MutationSet<HIV> mutations) {
		return mutations.getPrevalences()
			.entrySet()
			.stream()
			.map(e -> {
				Mutation<HIV> mainMut = e.getKey();
				Map<Mutation<HIV>, Map<String, Object>>	allResults = (
					e.getValue()
					.stream()
				...
				for (Mutation<HIV> singleAA : mainMut.split()) {
	...
	private static DataFetcher<List<Map<String, Object>>> subtypeStatsDataFetcher = env -> {
		String subtype = env.getSource();
		HIV hivObj = HIV.getInstance();
		return hivObj.getNumPatientsForAAPercents(hivObj.getStrain("HIV1"))
			.entrySet().stream()
			.map(e -> {
				Gene<HIV> gene = e.getKey();
	...
	public static GraphQLCodeRegistry mutationPrevalenceSubtypeCodeRegistry = newCodeRegistry()
		.dataFetcher(
