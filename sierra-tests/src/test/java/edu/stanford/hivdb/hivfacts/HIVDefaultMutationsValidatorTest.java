...
public class HIVDefaultMutationsValidatorTest {
	...
	public void testValidate() {
		...
		assertEquals("SEVERE_WARNING", results.get(0).getLevel().name());
		assertEquals(
			"The following 5 APOBEC muts were present in the sequence. The " +
			"following 3 DRMs in this sequence could reflect APOBEC activity: " +
			"PR: D30N, M46I, G48S.",
		...
		assertEquals("WARNING", results.get(0).getLevel().name());
		assertEquals(
			"The following 2 APOBEC muts were present in the sequence.", 
			results.get(0).getMessage()
		);
		...
		assertEquals(
			"The following 1 APOBEC muts were present in the sequence.",
			results.get(0).getMessage()
		);
